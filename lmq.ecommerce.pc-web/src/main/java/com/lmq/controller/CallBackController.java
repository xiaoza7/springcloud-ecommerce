...
import com.lmq.feignservice.CallBackServiceFegin;
...
public class CallBackController {
	...
	public void synCallBack(HttpServletRequest request,HttpServletResponse response) throws IOException {
		...
		for (Iterator<String> iter = requestParams.keySet().iterator(); iter.hasNext();) {
			...
			log.info("name is {},param is {}",name, valueStr);
			params.put(name, valueStr);
		}
		log.info("###支付宝同步回调CallBackController####synCallBack开始 params:{}", params);
		BaseResponse synCallBackResponseBase = callBackServiceFegin.synCallBack(params,"fhjfh");
		PrintWriter writer = response.getWriter();
		log.info("rreturn code is ："+synCallBackResponseBase.getRtnCode());
		if (!synCallBackResponseBase.getRtnCode().equals(Constants.HTTP_RES_CODE_200)) {
			// 报错页面
			return;
		}
		LinkedHashMap data = (LinkedHashMap) synCallBackResponseBase.getObj();
		...
		log.info("before我在支付宝同步回调response.write函数中 .....");
		writer.println(htmlFrom);	
		log.info("after我在支付宝同步回调response.write函数中 .....");
	...
	public String synSuccessPage(HttpServletRequest request, String outTradeNo, String tradeNo, String totalAmount) {
		...
		log.info("我在支付宝同步回调CallBackController####synCallBack跳转函数中 .....");
		return PAY_SUCCESS;
	}
